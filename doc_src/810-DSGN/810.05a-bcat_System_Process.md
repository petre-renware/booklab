...
The fie in format [markown format be accesed here](../bcat/bcat.md). Should remark that it contains some Jinja portions that are potected at 1st compilation by including them in sections `{% raw %} {% endraw %}` - this Jinja portions will be evaluated and rendered at execution (run time) running specific code.
