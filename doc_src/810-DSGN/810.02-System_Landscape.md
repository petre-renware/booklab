...
* **STATIC BookLab** - **serverul HTTP static** acesta va deservi partea de web compusa din "fisiere" statice. Acestea reprezinta fisierele HTML aferente interfetei UI dar si celelate fisiere "auxiliare" precum: CSS-uri, JS-uri, imagini, etc

* **DYNAMIC BookLab** - **serverul HTTP dinamic** ce opereaza in "background" (adica nu prezinta o interfata UI) si deserveste componentele care executa operatiile necesare (de ex create carte noua). Acest server este cunoscut si sub numele de `BookLab API`
...
!!! info "parametrii"
    * _code-name_: `bcat`
    * _locatie DYNAMIC BookLab_: temporary on `books_metainfo/` <!--#FIXME de lamurite daca aici sa numai sub serverul static, ref procesul de copiere ce pare articificial -->
