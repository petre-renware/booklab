...
* tbd...mv.me.to.ROADMAP... RMAP.001 - BCAT start for a new structure with python as CGI:
    ...
    * [ ] 3. the component template file `<component_name>.md` with raw Jinja to remain in markdown (qnd becomes .html after mkdoc compile) as subject to be rendered by python script
...
* ...230910piu_c RMAP.001:
    ...
    * [x] 3. the component template file `<component_name>.md` with raw Jinja to remain in markdown (qnd becomes .html after mkdoc compile) as subject to be rendered by python script, so old `index.md` renamed to `bcat.md` 
